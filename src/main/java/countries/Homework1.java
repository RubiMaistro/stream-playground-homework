...
public class Homework1 {
    ...
    public void streamPipeline3() {
        countries.stream()
            .filter(country -> country.getName()
                    .toLowerCase()
                    .charAt(0) == country.getName()
                            .toLowerCase()
                            .charAt(country.getName().length()-1))
            .map(Country::getName)
    ...
    public IntSummaryStatistics streamPipeline6() {
        return countries.stream()
                .mapToInt(country 
                    -> (int) country.getTimezones().stream()
                        .count())
                .summaryStatistics();
    }
    ...
    public void streamPipeline7() {
        countries.stream()
                .sorted(Comparator.comparing(country 
                        -> country.getTimezones().stream().count()))
                .map(Country::getName)
                .forEach(System.out::println);
    }
...
ed by the language code "es").
    ...
    public long streamPipeline9() {
        return countries.stream()
                .filter(country -> !country.getTranslations()
                        .containsKey("es"))
                .map(Country::getName)
                .count();
    }
    ...
    public void streamPipeline13() {
        countries.stream()
                .filter(country -> country.getArea() == null)
                .flatMap(country -> country.getRegion()
                        .describeConstable().stream())
                .distinct()
                .forEach(System.out::println);
    }
    ...
    public void streamPipeline16() {
        countries.stream()
                .filter(country
                        -> country.getRegion() == Region.EUROPE
